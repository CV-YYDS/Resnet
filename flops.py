...
#from models.modeling_resnet import StdConv2d
...
def std_conv_flops_counter_hook(conv_module, input, output):
    # Can have multiple inputs, getting the first one
    input = input[0]

    std_weight_flops = int(np.prod(conv_module.weight.shape))
    ...
    overall_flops = overall_conv_flops + bias_flops + std_weight_flops

    conv_module.__flops__ += int(overall_flops)


def compute_softmax_flops(multihead_attention_module, input, macs=False):
    input = input[0]
    print(input.shape)
    nfeatures = input[1:].numel()
...
MODULES_MAPPING = {
    ...
    # std convolutions
    #StdConv2d: std_conv_flops_counter_hook,
    # GroupNorm
    nn.GroupNorm: norm_flops_counter_hook,
    # LayerNorm
    nn.LayerNorm: norm_flops_counter_hook,
    nn.Softmax: compute_softmax_flops
